...
import be.frol.game.model.LostInTranslationRound.RoundType.RoundType
...
case class LitRichRound(round: LitRoundRow, roundType: Option[LostInTranslationRound.RoundType.Value]){
  ...
  def nextPlayer(litg: LitRichGame): RichUser = {
    Stream(litg.sortedUsers, litg.sortedUsers).flatten.dropWhile(_.u.id != round.fkUserId).drop(1).head
  }
}
